// api/create-asn.js
//
// Creates a new ASN as a TAB inside a brand's own ASN Google Sheet —
// NOT a separate Drive file. Per Jaclyn, 2026-07-27.
//
// MULTI-BRAND — added 2026-08-19. Originally hardcoded to évolis's own
// ASN spreadsheet only. Skinuva's dashboard was found to be calling this
// SAME endpoint with no way to tell it apart from évolis's — it would
// have created every skinuva ASN inside évolis's spreadsheet by mistake
// (confirmed: skinuva-index.html had évolis's TRACKER_SHEET_ID hardcoded
// directly in its own frontend JS). Now accepts an optional
// `trackerSheetId` in the request body; defaults to évolis's ID if not
// provided, so évolis's own dashboard (which doesn't send this
// parameter) is completely unaffected by this change. Any future brand
// just needs its dashboard to pass its own ASN sheet's ID — no new
// backend file needed, since the actual logic (add a tab, format it,
// write the tracker row) is identical for every brand.
//
...
//     tab, so once the Medaltus ASN script is pasted into a given
//     spreadsheet's Extensions > Apps Script ONCE, every tab in that
//     file — past, present, future — automatically has the same menu.
//     No copying, no Apps Script API, no domain-wide delegation, none of
//     it needed. NOTE: this needs to be confirmed done for skinuva's own
//     ASN spreadsheet specifically — it was only ever confirmed for
//     évolis's.
//   - No more separate-file Drive permissions to manage. Sharing is a
//     per-FILE setting; as long as a brand's ASN spreadsheet is already
//     shared "Anyone with the link: Viewer" (needed for the dashboard's
//     public CSV reads), every new tab automatically inherits that, with
//     nothing extra to grant per-brand.
...
//
// Still writes a summary row to the Inbound_Shipments tab in the SAME
// spreadsheet (unchanged), with a DriveUrl that points directly at the
// new tab's own gid — e.g.
//   https://docs.google.com/spreadsheets/d/{trackerSheetId}/edit#gid={newSheetId}
// — so the dashboard card's link opens straight to the right tab instead
// of a separate file.
//
// Manually-triggered from a dashboard's "Create ASN" button — no
...
const DEFAULT_TRACKER_SHEET_ID = '1Pb50CzCb0fouNsaewQATEY_c3IpgRVPu5FoppNg19_A'; // évolis — kept as the default for backward compatibility with évolis's own dashboard, which doesn't send trackerSheetId at all
...
module.exports = async function handler(req, res) {
  ...
  const { rows, trackerSheetId } = req.body || {};
  const TRACKER_SHEET_ID = trackerSheetId || DEFAULT_TRACKER_SHEET_ID;
