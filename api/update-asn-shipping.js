...
//
// MULTI-BRAND — added 2026-08-19, same reasoning and pattern as
// create-asn.js's identical change (see that file's header for the full
// explanation of why this was needed — skinuva's dashboard was calling
// this same endpoint with évolis's sheet ID hardcoded into its own
// frontend). Accepts an optional `trackerSheetId` in the request body;
// defaults to évolis's ID if not provided, so évolis's own dashboard is
// completely unaffected.
//
// Manually-triggered from a dashboard's "Add shipping info" form — no
// CRON_SECRET, same exception as run-analysis.js / create-asn.js.
//
// UPDATED 2026-07-27: every ASN now lives as a TAB inside a brand's own
// ASN spreadsheet instead of a separate Drive file — see create-asn.js
// for the full reasoning. That means the header-cell writes below now
// target `'${shipmentId}'!B6` etc. (a specific tab within the shared
// spreadsheet) rather than an unqualified 'B6' on a separate file's only
// tab. An unqualified range would default to whichever tab happens to be
// first/active in the spreadsheet — wrong now that there are many ASN
// tabs living together in one file. Since the tab name is always exactly
// the shipmentId (set at creation in create-asn.js), no URL-parsing is
// needed to find it — shipmentId is already the request's primary key.
...
module.exports = async function handler(req, res) {
  ...
  const { shipmentId, carrier, trackingNo, boxCount, trackerSheetId } = req.body || {};
